...
This analysis investigates emotional expression in social media posts authored by individuals self-identifying with anxiety or bipolar disorder. Two dictionary-based sentiment analysis methods were applied to Reddit and Twitter datasets: the sentimentr package and the NRC Emotion Intensity Lexicon.

## Datasets

Together, the following datasets allow us to compare emotional language across disorders and platforms, while also exposing trade-offs between breadth (Twitter) and depth (Reddit).

### Reddit

This dataset consists of 3,686 comments collected from six mental health-focused subreddits (three for anxiety, three for bipolar). The longer format of Reddit comments allows for more detailed emotional expression and context. However, subreddit membership is only a proxy for diagnosis, and not all comments are directly related to the disorder.

### Twitter

This dataset, sourced from Kaggle, includes over 9 million tweets from users who self-identify as diagnosed with either anxiety or bipolar disorder. Tweets are short and spontaneous, providing a broad view of public, self-labeled mental health expression. While the dataset is large and labeled, it relies entirely on self-identification and lacks contextual depth due to tweet length.

## Dictionaries

In this analysis, we applied two dictionary-based sentiment methods to examine emotional expression in Reddit and Twitter data related to anxiety and bipolar disorder: sentimentr and the NRC Emotion Intensity Lexicon.

### Sentimentr

 The sentimentr package computes sentiment scores by considering not just the presence of polarized words but also their context—specifically the effect of valence shifters such as negators (e.g., "not"), amplifiers (e.g., "very"), and de-amplifiers (e.g., "barely"). This method is beneficial for identifying subtle emotional tone shifts. However, it aggregates sentiment to a general positive/negative score, making it less suitable for analyzing discrete emotions like sadness or fear, which are critical in psychological research.

### NRC Emotion Intensity Lexicon

This lexicon assigns intensity values (0–1) to words associated with five basic emotions (anger, joy, fear, sadness, and disgust), allowing fine-grained measurement of specific emotional content. By applying the lexicon through quanteda.sentiment, we obtained emotion-specific scores at the document level. Its main advantage is the ability to distinguish among emotional dimensions, but it lacks context sensitivity; the presence of an emotional word triggers a score regardless of negation or sentence structure.

### **Comparison**

While sentimentr is great in contextual sentiment detection and reflects linguistic nuance, it offers only a binary affective valence. Conversely, the NRC Emotion Intensity lexicon captures a richer emotional profile but at the cost of ignoring syntactic modifiers. Together, they complement each other—sentimentr provides robustness to context, and NRC offers granularity in emotional categories.

## Hypotheses

1.  **Sentimentr:** Texts associated with bipolar disorder will show greater variance in valence (positive/negative sentiment) compared to anxiety-related texts, reflecting mood instability characteristic of bipolar disorder.
2.  **NRC Emotion Intensity Lexicon:** Emotion intensity values (0–1) will be lower overall in anxiety-related texts, particularly on Reddit, under the assumption that this community includes more casual or undiagnosed participants expressing milder emotional content.
...
## Dictionary Method 1: Sentimentr
...
## Dictionary Method 2: NRC Emotion Intensity

### Prepare Quanteda Corpus & DFM
...
### Creating function & connecting to git
...
#negative binomal it appears in the "book". it should be done between the groups and not between the features HADAS
...
#IN CASE IT IS SIGNIFICANT ADD SOMETHING TO SHOW WITHIN THE GRAPH
...
The average value for each emotion by group.
...
No significant group differences were found in either Reddit or Twitter using the sentimentr dictionary.

## NRC Emotion Intensity

We analyzed five emotions (anger, joy, fear, sadness, disgust) using linear regressions predicting emotion intensity from group (anxiety vs. bipolar), and a logistic regression predicting group based on emotion intensity.

### Reddit

The bipolar group expressed significantly higher levels of *disgust* compared to the anxiety group (*p* \< .001), while the anxiety group exhibited significantly more *sadness* and *fear* (*p* \< .05 for both). No significant differences were observed between the groups in terms of *anger* or *joy*.

To further explore these distinctions, a logistic regression model was conducted to predict group membership based on the five emotions. The model showed that *higher levels of disgust* significantly predicted classification into the bipolar group (estimate = 25.34, *p* \< .001), while *higher levels of fear* significantly predicted classification into the anxiety group (estimate = -15.23, *p* \< .001).

Overall, the regression findings suggest that disgust and fear are the most reliable emotional markers differentiating the two groups in Reddit discourse.

### Twitter

Fear was the only emotion that significantly differed between the groups in both models, with higher fear levels predicting bipolar posts (logistic p = 0.007; linear p = 0.0025). Disgust was marginally significant in the logistic model (p = 0.085), suggesting it might reach significance with a larger sample size. Other emotions (anger, joy, sadness) showed no significant differences. These findings suggest fear may be a linguistic marker distinguishing bipolar from anxiety in twitter.

When comparing the results across datasets, we observed opposite trends, suggesting that platform context (Twitter vs. Reddit) may strongly influence linguistic patterns. Contrary to our hypothesis, we found no significant variance differences between the groups using the sentimentr dictionary. Despite the clinical expectation of mood swings and emotional extremes in bipolar disorder, our analysis did not reflect this pattern.

In both models, only *fear* emerged as a consistent and significant predictor. Disgust approached significance in the logistic model, suggesting that with a larger sample size it might have reached significance.
